...
from utils import robust_request
...
def list_emails(service, user_id='me', query='', max_results=100):
    ...
    try:
        ...
        while 'nextPageToken' in response and len(messages) < max_results:
            ...
            if 'messages' in response:
                messages.extend(response['messages'])
            logger.info(f'Retrieved {len(messages)} emails so far')
        
        logger.info(f'Total emails retrieved: {len(messages)}')
        return messages[:max_results]
    except Exception as e:
        logger.error(f'An error occurred while listing emails: {e}')
        return []
...
def get_email_details(service, msg_id, user_id='me'):
    ...
    try:
        ...
        logger.info(f'Fetched email ID: {msg_id}')
        return email_data
    except Exception as e:
        logger.error(f'An error occurred while fetching email {msg_id}: {e}')
        return {}